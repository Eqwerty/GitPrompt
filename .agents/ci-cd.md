...
2. **`publish` job** (`needs: build`, calls `publish.yml`) — matrix-builds unsigned binaries for `linux-x64`/`osx-x64`/`osx-arm64`/`win-x64` (amd64 except `osx-arm64`, which is native since `macos-latest` is Apple Silicon hardware — see [platform.md](platform.md)) via `dotnet publish` and `.github/scripts/publish-artifact.sh` (which stamps `-p:SourceRevisionId="$(git rev-parse --short HEAD)"` and packages a `.tar.gz`/`.zip`). **No tests run here** — this job only cross-compiles (or, for `osx-arm64`, builds natively). Each platform's archive is uploaded as a build artifact.
