...
- `CacheRuntimeState`: a small type each cache class holds one instance of (`GitRepositorySharedCache.RuntimeState` / `GitStatusSharedCache.RuntimeState`), factoring out the logic both caches share beyond the record format itself:
  - `TryCleanupStaleEntries` — the opportunistic 5-minute cleanup throttle described above. The "next cleanup due" timestamp is persisted to a `cleanup-schedule.token` marker file in the cache's own directory (read via a plain `File.ReadAllText`, written via `WriteAtomically`) — **not** an in-memory field. This matters because `gitprompt` is a brand-new process on every render (see above): an in-memory-only timestamp would reset to "due now" on every single invocation and never actually throttle anything. Mirrors the `status-invalidation.token` pattern below.
