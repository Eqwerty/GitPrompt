...
- Stale entries (untouched for 7 days) are swept opportunistically, at most once per 5 minutes, via a persisted "next cleanup due" timestamp (`SharedCacheUtilities.CacheRuntimeState.TryCleanupStaleEntries`, see below) — there is no background timer; cleanup only runs as a side effect of a `Set` call.
...
- `CleanupStaleEntries`: deletes `*.cache` files whose last-write time is older than the stale threshold. Called from each cache's `CacheRuntimeState.TryCleanupStaleEntries`, not a shared timer.
...
- `CacheRuntimeState`: a small type each cache class holds one instance of (`GitRepositorySharedCache.RuntimeState` / `GitStatusSharedCache.RuntimeState`), factoring out the logic both caches share beyond the record format itself:
  - `TryCleanupStaleEntries` — the opportunistic 5-minute cleanup throttle described above. The "next cleanup due" timestamp is persisted to a `cleanup-schedule.token` marker file in the cache's own directory (read via a plain `File.ReadAllText`, written via `AtomicFileWriter.WriteAtomically`) — **not** an in-memory field. This matters because `gitprompt` is a brand-new process on every render (see above): an in-memory-only timestamp would reset to "due now" on every single invocation and never actually throttle anything. Mirrors the `status-invalidation.token` pattern below.
  - `GetCacheDirectoryPath` / `OverrideCacheDirectoryForTesting` / `OverrideTimeProviderForTesting` / `ResetCleanupScheduleForTesting` — the cache-directory and clock indirection, and their testing overrides.

`AtomicFileWriter` (`src/GitPrompt/AtomicFileWriter.cs`, root `GitPrompt` namespace — not git-specific) is the actual atomic-write primitive: write to a random temp file, then `File.Move(overwrite: true)`. Prevents another concurrently-running `gitprompt` process from reading a half-written file. Originally lived inside `SharedCacheUtilities` and was used only by the two caches; moved out and given a `string`-content overload (alongside the original `string[]` lines overload) once `Configuration/ConfigInitializer.cs` and `Commands/ConfigResetCommand.cs` needed the same atomicity guarantee for writing `config.jsonc` — a plain `File.WriteAllText` there could otherwise be torn by two `gitprompt` processes racing on the same file (e.g. a terminal multiplexer restoring several panes right after an install/upgrade that triggers a config migration).

Both cache classes expose `OverrideTimeProviderForTesting`, `OverrideCacheDirectoryForTesting`, and `ResetCleanupScheduleForTesting` (delegating to their `CacheRuntimeState` instance) — internal-only static seams for deterministic unit tests. Use these instead of reflection or wall-clock sleeps when writing new cache tests; see existing tests in `tests/GitPrompt.Tests.Unit/Git/` for the pattern, and [testing.md](testing.md) for the `ConfigIsolationCollection` marker that disables parallelization for tests that touch this shared static state.
