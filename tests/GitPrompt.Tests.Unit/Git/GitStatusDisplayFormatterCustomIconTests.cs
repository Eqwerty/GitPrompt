...
public sealed class GitStatusDisplayFormatterCustomIconTests
{
    ...
    public void BuildBranchLabel_WhenDetachedBranchLabelOpenIsConfigured_ShouldUseItOnlyForDetachedState()
    {
        ...
        var normalLabel = GitStatusDisplayFormatter.BuildBranchLabel("main", BranchState.Normal);

        // Assert
        detachedLabel.Label.Should().StartWith(":<");
        detachedLabel.Label.Should().EndWith(">");
        normalLabel.Label.Should().Be("(main)");
    }
