...
| `gitprompt aliases` | Open git_aliases.sh in `$EDITOR` (if installed) |
| `gitprompt aliases enable` | Load git aliases into the current shell session |
| `gitprompt aliases disable` | Remove git aliases from the current shell session |
