namespace Prompt.Constants;

internal static class PromptColors
{
    private const string ReadLineStart = "\u0001";
    private const string ReadLineEnd = "\u0002";

    private static string PromptColor(string ansiColor) => $"{ReadLineStart}{ansiColor}{ReadLineEnd}";

    internal static readonly string ColorUser = PromptColor(AnsiColors.Green);
    internal static readonly string ColorHost = PromptColor(AnsiColors.BoldMagenta);
    internal static readonly string ColorPath = PromptColor(AnsiColors.Orange);
    internal static readonly string ColorBranch = PromptColor(AnsiColors.BoldCyan);
    internal static readonly string ColorBranchNoUpstream = PromptColor(AnsiColors.BoldCyan);
    internal static readonly string ColorAhead = PromptColor(AnsiColors.BoldCyan);
    internal static readonly string ColorBehind = PromptColor(AnsiColors.BoldCyan);
    internal static readonly string ColorStaged = PromptColor(AnsiColors.Green);
    internal static readonly string ColorUnstaged = PromptColor(AnsiColors.Red);
    internal static readonly string ColorUntracked = PromptColor(AnsiColors.Red);
    internal static readonly string ColorStash = PromptColor(AnsiColors.BoldMagenta);
    internal static readonly string ColorState = PromptColor(AnsiColors.BoldRed);
    internal static readonly string ColorPrompt = PromptColor(AnsiColors.LightGray);
    internal static readonly string ColorReset = PromptColor(AnsiColors.Reset);
}
